...
// YadorePublisher SDK pipeline test
//
...
{
    private static function utility(): YadorePublisherUtility
    {
        return new YadorePublisherUtility();
    }
    ...
    private static function ctx(array $over = []): YadorePublisherContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new YadorePublisherContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new YadorePublisherOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof YadorePublisherError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new YadorePublisherOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = YadorePublisherMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new YadorePublisherOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = YadorePublisherMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new YadorePublisherOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = YadorePublisherMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new YadorePublisherSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = YadorePublisherMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new YadorePublisherResponse([]),
            'result' => new YadorePublisherResult([]),
        ]);
        [, $err] = YadorePublisherMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new YadorePublisherSpec([]),
            'response' => null,
            'result' => new YadorePublisherResult([]),
        ]);
        [, $err] = YadorePublisherMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new YadorePublisherSpec(['step' => 's']),
            'response' => new YadorePublisherResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new YadorePublisherResult([]),
        ]);
        [, $err] = YadorePublisherMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new YadorePublisherSpec(['step' => 's']),
            'response' => new YadorePublisherResponse(self::resp(200, ['v' => 1])),
            'result' => new YadorePublisherResult([]),
        ]);
        [, $err] = YadorePublisherMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new YadorePublisherSpec(['step' => 's']),
            'response' => new YadorePublisherResponse(self::resp(200, ['v' => 2])),
            'result' => new YadorePublisherResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        YadorePublisherMakeResponse::call($ctx);
    ...
    {
        $preset = new YadorePublisherResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new YadorePublisherSpec([]),
            'response' => new YadorePublisherResponse([]),
            'result' => new YadorePublisherResult([]),
        ...
        [$got, $err] = YadorePublisherMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new YadorePublisherResult([])]);
        [, $err] = YadorePublisherMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new YadorePublisherSpec([]), 'result' => null]);
        [, $err] = YadorePublisherMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new YadorePublisherSpec(['step' => 's']),
            'result' => new YadorePublisherResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new YadorePublisherOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = YadorePublisherMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new YadorePublisherSpec(['step' => 's']),
            'result' => new YadorePublisherResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new YadorePublisherOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = YadorePublisherMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new YadorePublisherResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new YadorePublisherSpec([]),
            'result' => new YadorePublisherResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = YadorePublisherMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = YadorePublisherMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new YadorePublisherError('boom', 'boom');
        $utility->fetcher = function (YadorePublisherContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new YadorePublisherSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = YadorePublisherMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (YadorePublisherContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = YadorePublisherMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(YadorePublisherResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        YadorePublisherMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (YadorePublisherContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new YadorePublisherSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = YadorePublisherMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new YadorePublisherResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new YadorePublisherSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = YadorePublisherMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = YadorePublisherMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = YadorePublisherMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new YadorePublisherResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, YadorePublisherDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new YadorePublisherResult(['ok' => false])]);
        $this->expectException(YadorePublisherError::class);
        YadorePublisherDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new YadorePublisherResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', YadorePublisherMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        YadorePublisherMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new YadorePublisherBaseFeature();
        $b = new YadorePublisherBaseFeature();
        YadorePublisherFeatureAdd::call($ctx, $a);
        YadorePublisherFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): YadorePublisherBaseFeature
    {
        $f = new YadorePublisherBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        YadorePublisherFeatureAdd::call($ctx, self::named_feature('a'));
        YadorePublisherFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        YadorePublisherFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        YadorePublisherFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        YadorePublisherFeatureAdd::call($ctx, $replace);
        ...
        YadorePublisherFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): YadorePublisherContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new YadorePublisherSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = YadorePublisherPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        YadorePublisherPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new YadorePublisherResponse(['headers' => null]),
            'result' => new YadorePublisherResult([]),
        ]);
        YadorePublisherResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        YadorePublisherResultBody::call($ctx);
