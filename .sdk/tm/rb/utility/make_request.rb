...
module YadorePublisherUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = YadorePublisherResponse.new({})
    ...
    if fetch_err
      ...
      response = YadorePublisherResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = YadorePublisherResponse.new(fetched)
    else
