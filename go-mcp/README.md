# yadore-publisher-mcp

[MCP](https://modelcontextprotocol.io) server exposing the YadorePublisher SDK as
two agent tools — `yadore-publisher_list` and `yadore-publisher_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/yadore-publisher-mcp)
make build

# 2. Provide credentials via the environment
export YADORE_PUBLISHER_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user yadore-publisher \
  -- /absolute/path/to/yadore-publisher-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./yadore-publisher-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// yadore-publisher_list: first page of records
{ "entity": "conversion_detail" }
{ "entity": "conversion_detail", "query": { } }

// yadore-publisher_load: one record by id
{ "entity": "conversion_general", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/yadore-publisher-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user yadore-publisher \
     -- "$PWD"/dist/*/yadore-publisher-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `yadore-publisher_list` and `yadore-publisher_load` tools now appear
   in new sessions. Ask the agent to *"list conversion_detail using yadore-publisher"*
   and it calls `yadore-publisher_list` with `{"entity":"conversion_detail"}`.
...
```sh
export YADORE_PUBLISHER_APIKEY=sk_live_xxx            # API key
export YADORE_PUBLISHER_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `yadore-publisher_list` tool
...
### Call the `yadore-publisher_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `yadore-publisher_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `yadore-publisher_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `YADORE_PUBLISHER_APIKEY` | API key sent with every request. |
| `YADORE_PUBLISHER_BASE` | Optional override of the API base URL. |

### Entities

The 14 entities valid as the `entity` argument:

conversion_detail | conversion_detail_merchant | conversion_general | conversion_status | deeplink | deeplink_merchant | dnt | market | merchant | offer | report_detail | report_general | report_modified | report_status

### Smoke test via HTTP (raw JSON-RPC)

```sh
./yadore-publisher-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"yadore-publisher_load","arguments":{"entity":"conversion_general","query":{"id":1}}}}'
```
