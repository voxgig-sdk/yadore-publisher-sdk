...
class TestConversionDetailMerchantDirect:

    def test_should_direct_list_conversion_detail_merchant(self):
        setup = _conversion_detail_merchant_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-conversion_detail_merchant", "live" if setup["live"] else "unit")
        if _skip:
...
def _conversion_detail_merchant_direct_setup(mockres):
