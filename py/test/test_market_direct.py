...
class TestMarketDirect:
    ...
    def test_should_direct_list_market(self):
        setup = _market_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-market", "live" if setup["live"] else "unit")
        if _skip:
