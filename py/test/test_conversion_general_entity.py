    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set YADOREPUBLISHER_TEST_CONVERSION_GENERAL_ENTID JSON to run live")
        client = setup["client"]
        ...
        conversion_general_ref01_data_dt0_loaded, err = conversion_general_ref01_ent.load(conversion_general_ref01_match_dt0, None)
        assert err is None
...
def _conversion_general_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("YADOREPUBLISHER_TEST_CONVERSION_GENERAL_ENTID"))
