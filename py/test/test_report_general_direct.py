...
class TestReportGeneralDirect:

    def test_should_direct_load_report_general(self):
        setup = _report_general_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-report_general", "live" if setup["live"] else "unit")
        if _skip:
...
def _report_general_direct_setup(mockres):
