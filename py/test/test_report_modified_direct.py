...
class TestReportModifiedDirect:

    def test_should_direct_load_report_modified(self):
        setup = _report_modified_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-report_modified", "live" if setup["live"] else "unit")
        if _skip:
...
def _report_modified_direct_setup(mockres):
