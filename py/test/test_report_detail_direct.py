...
class TestReportDetailDirect:

    def test_should_direct_list_report_detail(self):
        setup = _report_detail_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-report_detail", "live" if setup["live"] else "unit")
        if _skip:
...
def _report_detail_direct_setup(mockres):
