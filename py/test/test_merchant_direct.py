...
class TestMerchantDirect:

    def test_should_direct_list_merchant(self):
        setup = _merchant_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-merchant", "live" if setup["live"] else "unit")
        if _skip:
...
def _merchant_direct_setup(mockres):
