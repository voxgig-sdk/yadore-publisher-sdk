# YadorePublisher SDK test runner
...
class YadorePublisherTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            YadorePublisherTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = YadorePublisherTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = YadorePublisherTestRunner.getenv("YADORE_PUBLISHER_TEST_LIVE")
        override = YadorePublisherTestRunner.getenv("YADORE_PUBLISHER_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = YadorePublisherTestRunner.getenv(key)
        ...
        explain = YadorePublisherTestRunner.getenv("YADORE_PUBLISHER_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["YADORE_PUBLISHER_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if YadorePublisherTestRunner._test_control is not None:
            return YadorePublisherTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                YadorePublisherTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            YadorePublisherTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = YadorePublisherTestRunner.load_test_control()
...
def load_env_local():
    YadorePublisherTestRunner.load_env_local()


def env_override(m):
    return YadorePublisherTestRunner.env_override(m)


def entity_data(v):
    return YadorePublisherTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return YadorePublisherTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return YadorePublisherTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return YadorePublisherTestRunner.load_test_control()


def live_client_options():
    return YadorePublisherTestRunner.live_client_options()


def live_delay_ms():
    return YadorePublisherTestRunner.live_delay_ms()
