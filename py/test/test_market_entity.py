...
class TestMarketEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Market(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Market(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set YADORE_PUBLISHER_TEST_MARKET_ENTID JSON to run live")
        client = setup["client"]
        ...
        market_ref01_list_result = market_ref01_ent.list(market_ref01_match, None)
        assert isinstance(market_ref01_list_result, list)
...
def _market_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "YADORE_PUBLISHER_TEST_MARKET_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("YADORE_PUBLISHER_TEST_MARKET_ENTID"))
