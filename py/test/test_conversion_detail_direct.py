...
class TestConversionDetailDirect:

    def test_should_direct_list_conversion_detail(self):
        setup = _conversion_detail_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-conversion_detail", "live" if setup["live"] else "unit")
        if _skip:
...
def _conversion_detail_direct_setup(mockres):
