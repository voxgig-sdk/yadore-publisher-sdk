...
class TestDeeplinkEntity:

    def test_should_create_instance(self):
        testsdk = YadorePublisherSDK.test(None, None)
        ent = testsdk.Deeplink(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _deeplink_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "deeplink." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set YADORE_PUBLISHER_TEST_DEEPLINK_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        deeplink_ref01_ent = client.Deeplink(None)
        deeplink_ref01_data = helpers.to_map(vs.getprop(
            vs.getpath(setup["data"], "new.deeplink"), "deeplink_ref01"))

        deeplink_ref01_data = helpers.to_map(runner.entity_data(deeplink_ref01_ent.create(deeplink_ref01_data, None)))
        assert deeplink_ref01_data is not None



def _deeplink_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/deeplink/DeeplinkTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "YADORE_PUBLISHER_TEST_DEEPLINK_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("YADORE_PUBLISHER_TEST_DEEPLINK_ENTID"))
