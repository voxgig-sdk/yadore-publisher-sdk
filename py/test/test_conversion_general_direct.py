...
class TestConversionGeneralDirect:

    def test_should_direct_load_conversion_general(self):
        setup = _conversion_general_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-conversion_general", "live" if setup["live"] else "unit")
        if _skip:
...
def _conversion_general_direct_setup(mockres):
