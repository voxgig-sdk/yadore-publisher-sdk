...
class ReportModifiedEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ReportModifiedEntity(self._client, opts)
    ...
    def data_get(self) -> ReportModified:
    ...
    def match_get(self) -> ReportModified:
    ...
    def load(self, reqmatch=None, ctrl=None) -> ReportModified:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.ReportModified().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
