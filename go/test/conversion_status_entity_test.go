...
func TestConversionStatusEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		conversionStatusRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.conversion_status")))
...
func conversion_statusBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("YADORE_PUBLISHER_TEST_CONVERSION_STATUS_ENTID")
	...
	idmapResolved := core.ToMapAny(env["YADORE_PUBLISHER_TEST_CONVERSION_STATUS_ENTID"])
