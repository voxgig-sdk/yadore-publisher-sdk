...
func TestDeeplinkMerchantEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		deeplinkMerchantRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.deeplink_merchant")))
...
func deeplink_merchantBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("YADORE_PUBLISHER_TEST_DEEPLINK_MERCHANT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["YADORE_PUBLISHER_TEST_DEEPLINK_MERCHANT_ENTID"])
