...
func TestDeeplinkEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set YADOREPUBLISHER_TEST_DEEPLINK_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		deeplinkRef01Data = core.ToMapAny(deeplinkRef01DataResult)
...
func deeplinkBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("YADOREPUBLISHER_TEST_DEEPLINK_ENTID")
