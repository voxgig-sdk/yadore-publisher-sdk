...
func TestMerchantEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set YADORE_PUBLISHER_TEST_MERCHANT_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		merchantRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.merchant")))
...
func merchantBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("YADORE_PUBLISHER_TEST_MERCHANT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["YADORE_PUBLISHER_TEST_MERCHANT_ENTID"])
