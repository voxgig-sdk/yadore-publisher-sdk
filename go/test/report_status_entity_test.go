...
func TestReportStatusEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		reportStatusRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.report_status")))
...
func report_statusBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("YADORE_PUBLISHER_TEST_REPORT_STATUS_ENTID")
	...
	idmapResolved := core.ToMapAny(env["YADORE_PUBLISHER_TEST_REPORT_STATUS_ENTID"])
