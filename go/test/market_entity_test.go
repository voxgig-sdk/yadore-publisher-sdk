...
func TestMarketEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		marketRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.market")))
...
func marketBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("YADORE_PUBLISHER_TEST_MARKET_ENTID")
	...
	idmapResolved := core.ToMapAny(env["YADORE_PUBLISHER_TEST_MARKET_ENTID"])
