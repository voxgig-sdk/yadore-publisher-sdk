...
func TestOfferEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Offer(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Offer(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set YADORE_PUBLISHER_TEST_OFFER_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		offerRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.offer")))
		...
		offerRef01DataDt0LoadResult := core.ToMapAny(entityData(offerRef01DataDt0Loaded))
...
func offerBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("YADORE_PUBLISHER_TEST_OFFER_ENTID")
	...
	idmapResolved := core.ToMapAny(env["YADORE_PUBLISHER_TEST_OFFER_ENTID"])
