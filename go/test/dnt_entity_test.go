...
func TestDntEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		dntRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.dnt")))
...
func dntBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("YADORE_PUBLISHER_TEST_DNT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["YADORE_PUBLISHER_TEST_DNT_ENTID"])
