...
import (
	...
	omni "github.com/voxgig-sdk/yadore-publisher-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.YadorePublisherError); is {
				out := map[string]any{
					"name":    "YadorePublisherError",
...
func omniCtx(arg any, client *sdk.YadorePublisherSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.YadorePublisherSDK); is {
			client = live
