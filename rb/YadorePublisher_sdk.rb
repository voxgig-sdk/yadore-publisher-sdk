...
class YadorePublisherSDK
    ...
    if feature_opts
      ...
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = YadorePublisherHelpers.to_map(feature_opts[fname])
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = YadorePublisherError.new(
        "graphql_error", "YadorePublisherSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
