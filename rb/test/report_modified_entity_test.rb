...
class ReportModifiedEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set YADOREPUBLISHER_TEST_REPORT_MODIFIED_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    report_modified_ref01_data_dt0_loaded, err = report_modified_ref01_ent.load(report_modified_ref01_match_dt0, nil)
    assert_nil err
...
def report_modified_basic_setup(extra)
  ...
  entid_env_raw = ENV["YADOREPUBLISHER_TEST_REPORT_MODIFIED_ENTID"]
