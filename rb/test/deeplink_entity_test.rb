...
class DeeplinkEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set YADORE_PUBLISHER_TEST_DEEPLINK_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    deeplink_ref01_data = Helpers.to_map(deeplink_ref01_data_result.respond_to?(:data_get) ? deeplink_ref01_data_result.data_get : deeplink_ref01_data_result)
...
def deeplink_basic_setup(extra)
  ...
  entid_env_raw = ENV["YADORE_PUBLISHER_TEST_DEEPLINK_ENTID"]
