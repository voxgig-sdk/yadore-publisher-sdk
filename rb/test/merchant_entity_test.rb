...
class MerchantEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Merchant(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Merchant(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set YADORE_PUBLISHER_TEST_MERCHANT_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    merchant_ref01_data = nil
    if merchant_ref01_data_raw.length > 0
...
def merchant_basic_setup(extra)
  ...
  entid_env_raw = ENV["YADORE_PUBLISHER_TEST_MERCHANT_ENTID"]
  ...
  idmap_resolved = Helpers.to_map(
    env["YADORE_PUBLISHER_TEST_MERCHANT_ENTID"])
