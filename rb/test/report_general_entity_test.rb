...
class ReportGeneralEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set YADORE_PUBLISHER_TEST_REPORT_GENERAL_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    report_general_ref01_data_dt0_loaded = report_general_ref01_ent.load(report_general_ref01_match_dt0, nil)
...
def report_general_basic_setup(extra)
  ...
  entid_env_raw = ENV["YADORE_PUBLISHER_TEST_REPORT_GENERAL_ENTID"]
