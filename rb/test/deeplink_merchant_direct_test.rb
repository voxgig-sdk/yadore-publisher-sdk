...
class DeeplinkMerchantDirectTest < Minitest::Test
  def test_direct_list_deeplink_merchant
    setup = deeplink_merchant_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-deeplink_merchant", setup[:live] ? "live" : "unit")
    if _should_skip
...
def deeplink_merchant_direct_setup(mockres)
