...
class ConversionStatusEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = conversion_status_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "conversion_status." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    conversion_status_ref01_data = nil
    if conversion_status_ref01_data_raw.length > 0
...
def conversion_status_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "conversion_status", "ConversionStatusTestData.json")
