...
class ReportStatusEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = report_status_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "report_status." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set YADORE_PUBLISHER_TEST_REPORT_STATUS_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    report_status_ref01_data = nil
    if report_status_ref01_data_raw.length > 0
...
def report_status_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "report_status", "ReportStatusTestData.json")
  ...
  entid_env_raw = ENV["YADORE_PUBLISHER_TEST_REPORT_STATUS_ENTID"]
