...
class ReportDetailDirectTest < Minitest::Test
  def test_direct_list_report_detail
    setup = report_detail_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-report_detail", setup[:live] ? "live" : "unit")
    if _should_skip
...
def report_detail_direct_setup(mockres)
