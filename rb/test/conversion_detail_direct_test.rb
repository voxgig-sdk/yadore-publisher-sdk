...
class ConversionDetailDirectTest < Minitest::Test
  def test_direct_list_conversion_detail
    setup = conversion_detail_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-conversion_detail", setup[:live] ? "live" : "unit")
    if _should_skip
...
def conversion_detail_direct_setup(mockres)
