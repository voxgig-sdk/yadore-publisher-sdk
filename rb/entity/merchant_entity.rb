...
class MerchantEntity
  ...
  def make
    opts = @_entopts.dup
    MerchantEntity.new(@_client, opts)
  end
  ...
  # @return [Merchant, Hash] the current Merchant data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of Merchant fields)
  def match_get
  ...
  #
  # @param reqmatch [MerchantListMatch, Hash, nil] match filter (any subset of
  #   Merchant fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Merchant>, Array] the matching Merchant items; raises YadorePublisherError on failure
