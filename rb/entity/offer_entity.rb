...
class OfferEntity
  ...
  def make
    opts = @_entopts.dup
    OfferEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = YadorePublisherHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Offer, Hash] the current Offer data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = YadorePublisherHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Offer fields)
  def match_get
  ...
  # Load a single Offer.
  #
  # @param reqmatch [OfferLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Offer.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Offer, Hash] the loaded Offer; raises YadorePublisherError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = YadorePublisherHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [OfferListMatch, Hash, nil] match filter (any subset of
  #   Offer fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Offer>, Array] the matching Offer items; raises YadorePublisherError on failure
