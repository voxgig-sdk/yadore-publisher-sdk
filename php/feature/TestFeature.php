...
// YadorePublisher SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class YadorePublisherTestFeature extends YadorePublisherBaseFeature
{
    ...
    public function init(YadorePublisherContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (YadorePublisherContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = YadorePublisherParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (YadorePublisherContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(YadorePublisherContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = YadorePublisherParam::call($ctx, $k);
