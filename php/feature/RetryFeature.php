...
// YadorePublisher SDK retry feature
...
class YadorePublisherRetryFeature extends YadorePublisherBaseFeature
{
    ...
    public function init(YadorePublisherContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (YadorePublisherContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(YadorePublisherContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(YadorePublisherContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
