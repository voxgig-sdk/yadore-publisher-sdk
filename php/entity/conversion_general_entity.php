...
// YadorePublisher SDK ConversionGeneral entity
...
class ConversionGeneralEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new ConversionGeneralEntity($this->_client, $opts);
    }

    /**
     * @param ConversionGeneral|array $args ConversionGeneral data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = YadorePublisherHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return ConversionGeneral|array The current ConversionGeneral data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of ConversionGeneral fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = YadorePublisherHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of ConversionGeneral fields).
     */
    ...
    /**
     * Load a single ConversionGeneral.
     *
     * @param ConversionGeneralLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed ConversionGeneralLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return ConversionGeneral|array The loaded ConversionGeneral as an assoc-array at the
     *   SDK boundary; throws YadorePublisherError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = YadorePublisherHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
