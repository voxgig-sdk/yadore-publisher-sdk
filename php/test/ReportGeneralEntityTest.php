...
class ReportGeneralEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = YadorePublisherSDK::test(null, null);
        $ent = $testsdk->ReportGeneral(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = report_general_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "report_general." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set YADORE_PUBLISHER_TEST_REPORT_GENERAL_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $report_general_ref01_data = null;
        if (count($report_general_ref01_data_raw) > 0) {
...
function report_general_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/report_general/ReportGeneralTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["report_general01", "report_general02", "report_general03"] as $k) {
    ...
    $entid_env_raw = getenv("YADORE_PUBLISHER_TEST_REPORT_GENERAL_ENTID");
