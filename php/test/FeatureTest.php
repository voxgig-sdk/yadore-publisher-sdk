...
// YadorePublisher SDK feature test
//
...
class FtCtrl extends YadorePublisherControl
{
...
class FtClient
{
    ...
    private YadorePublisherUtility $utility;

    public function __construct(YadorePublisherUtility $utility, array $options)
    {
    ...
    public function get_utility(): YadorePublisherUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (YadorePublisherContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public YadorePublisherContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (YadorePublisherContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = YadorePublisherConfig::make_feature($name);
    ...
    private static function buildUrl(YadorePublisherSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof YadorePublisherError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new YadorePublisherResponse($fetched);
        }
    ...
    private function fail(YadorePublisherContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (YadorePublisherContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
