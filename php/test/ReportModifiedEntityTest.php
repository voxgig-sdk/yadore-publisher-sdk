...
class ReportModifiedEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = YadorePublisherSDK::test(null, null);
        $ent = $testsdk->ReportModified(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = report_modified_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "report_modified." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set YADOREPUBLISHER_TEST_REPORT_MODIFIED_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $report_modified_ref01_data = null;
        if (count($report_modified_ref01_data_raw) > 0) {
...
function report_modified_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/report_modified/ReportModifiedTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["report_modified01", "report_modified02", "report_modified03"] as $k) {
    ...
    $entid_env_raw = getenv("YADOREPUBLISHER_TEST_REPORT_MODIFIED_ENTID");
