...
class DeeplinkMerchantEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = YadorePublisherSDK::test(null, null);
        $ent = $testsdk->DeeplinkMerchant(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->DeeplinkMerchant(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->DeeplinkMerchant(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = deeplink_merchant_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "deeplink_merchant." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set YADORE_PUBLISHER_TEST_DEEPLINK_MERCHANT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $deeplink_merchant_ref01_data = null;
        if (count($deeplink_merchant_ref01_data_raw) > 0) {
...
function deeplink_merchant_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/deeplink_merchant/DeeplinkMerchantTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["deeplink_merchant01", "deeplink_merchant02", "deeplink_merchant03"] as $k) {
    ...
    $entid_env_raw = getenv("YADORE_PUBLISHER_TEST_DEEPLINK_MERCHANT_ENTID");
