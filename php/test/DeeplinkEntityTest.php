...
class DeeplinkEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = YadorePublisherSDK::test(null, null);
        $ent = $testsdk->Deeplink(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = deeplink_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "deeplink." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set YADORE_PUBLISHER_TEST_DEEPLINK_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $deeplink_ref01_ent = $client->Deeplink(null);
        ...
        $deeplink_ref01_data = Helpers::to_map(is_object($deeplink_ref01_data_result) && method_exists($deeplink_ref01_data_result, 'data_get') ? $deeplink_ref01_data_result->data_get() : $deeplink_ref01_data_result);
...
function deeplink_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/deeplink/DeeplinkTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["deeplink01", "deeplink02", "deeplink03"] as $k) {
    ...
    $entid_env_raw = getenv("YADORE_PUBLISHER_TEST_DEEPLINK_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["YADORE_PUBLISHER_TEST_DEEPLINK_ENTID"]);
