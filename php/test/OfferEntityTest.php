...
class OfferEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Offer(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Offer(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = offer_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "offer." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set YADORE_PUBLISHER_TEST_OFFER_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $offer_ref01_data = null;
        if (count($offer_ref01_data_raw) > 0) {
        ...
        $offer_ref01_data_dt0_load_result = Helpers::to_map(is_object($offer_ref01_data_dt0_loaded) && method_exists($offer_ref01_data_dt0_loaded, 'data_get') ? $offer_ref01_data_dt0_loaded->data_get() : $offer_ref01_data_dt0_loaded);
...
function offer_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/offer/OfferTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["offer01", "offer02", "offer03"] as $k) {
    ...
    $entid_env_raw = getenv("YADORE_PUBLISHER_TEST_OFFER_ENTID");
