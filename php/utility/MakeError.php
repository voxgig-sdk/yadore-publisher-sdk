...
class YadorePublisherMakeError
{
    public static function call(?YadorePublisherContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new YadorePublisherContext([], null);
        }
        $op = $ctx->op ?? new YadorePublisherOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new YadorePublisherResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof YadorePublisherError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new YadorePublisherError('', $msg, $ctx);
        ...
        if ($err instanceof YadorePublisherError) {
