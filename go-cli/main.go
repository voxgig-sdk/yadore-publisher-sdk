// Command YadorePublisher-cli is a CLI + REPL for the YadorePublisher SDK,
...
const prompt = "yadore-publisher"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "conversion_detail conversion_detail_merchant conversion_general conversion_status deeplink deeplink_merchant dnt market merchant offer report_detail report_general report_modified report_status"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      YADORE_PUBLISHER_APIKEY (api key), YADORE_PUBLISHER_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: YADORE_PUBLISHER_APIKEY carries the API key and
	// YADORE_PUBLISHER_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("YADORE_PUBLISHER_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("YADORE_PUBLISHER_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewYadorePublisherSDK(opts)
