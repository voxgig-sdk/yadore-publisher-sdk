# yadore-publisher-cli

boru-driven command-line client **and** interactive REPL for the YadorePublisher
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/yadore-publisher-cli)
make build

# 2. See usage (words, entities, env vars)
./yadore-publisher-cli --help

# 3. Provide credentials once, via the environment
export YADORE_PUBLISHER_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./yadore-publisher-cli list conversion_detail
./yadore-publisher-cli list conversion_detail_merchant

# 5. Override the API base URL for a single call
YADORE_PUBLISHER_BASE=https://api.example.com ./yadore-publisher-cli list conversion_detail

# 6. No arguments -> interactive REPL
./yadore-publisher-cli
yadore-publisher> list conversion_detail
yadore-publisher> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/yadore-publisher-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/yadore-publisher-cli list conversion_detail
   ```
...
Run with no arguments to open a REPL (prompt `yadore-publisher>`). Each line is
evaluated as its own boru expression:

```text
$ ./yadore-publisher-cli
yadore-publisher> list conversion_detail
yadore-publisher> /help
yadore-publisher> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `conversion_detail`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/yadore-publisher-cli`. |
...
(`./yadore-publisher-cli <expr>`) and interactively (the REPL), and expressions compose the
