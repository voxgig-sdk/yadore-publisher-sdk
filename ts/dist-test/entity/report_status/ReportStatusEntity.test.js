...
(0, node_test_1.describe)('ReportStatusEntity', async () => {
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        for (const op of ['load']) {
            if ((0, utility_1.maybeSkipControl)(t, 'entityOp', 'report_status.' + op, live))
                return;
        }
        ...
        if (setup.syntheticOnly) {
            t.skip('live entity test uses synthetic IDs from fixture — set YADORE_PUBLISHER_TEST_REPORT_STATUS_ENTID JSON to run live');
            return;
        }
        ...
        let report_status_ref01_data = Object.values(setup.data.existing.report_status)[0];
        // LOAD
        const report_status_ref01_ent = client.ReportStatus();
        const report_status_ref01_match_dt0 = {};
        const report_status_ref01_data_dt0 = (await report_status_ref01_ent.load(report_status_ref01_match_dt0)).data();
        (0, node_assert_1.default)(null != report_status_ref01_data_dt0);
    });
});
function basicSetup(extra) {
    ...
    const entityDataFile = node_path_1.default.resolve(__dirname, '../../../../.sdk/test/entity/report_status/ReportStatusTestData.json');
    ...
    let idmap = transform(['report_status01', 'report_status02', 'report_status03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    const idmapEnvVal = process.env['YADORE_PUBLISHER_TEST_REPORT_STATUS_ENTID'];
...
//# sourceMappingURL=ReportStatusEntity.test.js.map