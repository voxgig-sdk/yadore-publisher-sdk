...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.YADOREPUBLISHER_TEST_LIVE ||
    'TRUE' === process.env.YADOREPUBLISHER_TEST_OVERRIDE
  ) {
  ...
  m.YADOREPUBLISHER_TEST_EXPLAIN = process.env.YADOREPUBLISHER_TEST_EXPLAIN || m.YADOREPUBLISHER_TEST_EXPLAIN
