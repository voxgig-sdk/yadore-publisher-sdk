...
describe('ConversionDetailEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'conversion_detail.' + op, live)) return
    }
    ...
    const conversion_detail_ref01_list = await conversion_detail_ref01_ent.list(conversion_detail_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['YADORE_PUBLISHER_TEST_CONVERSION_DETAIL_ENTID']
