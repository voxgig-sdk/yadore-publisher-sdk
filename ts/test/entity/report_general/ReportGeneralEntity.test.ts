...
describe('ReportGeneralEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'report_general.' + op, live)) return
    }
    ...
    const report_general_ref01_data_dt0 = await report_general_ref01_ent.load(report_general_ref01_match_dt0)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['YADORE_PUBLISHER_TEST_REPORT_GENERAL_ENTID']
