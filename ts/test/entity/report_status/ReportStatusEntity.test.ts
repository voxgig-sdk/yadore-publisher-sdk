...
describe('ReportStatusEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'report_status.' + op, live)) return
    }
    ...
    const report_status_ref01_data_dt0 = await report_status_ref01_ent.load(report_status_ref01_match_dt0)
