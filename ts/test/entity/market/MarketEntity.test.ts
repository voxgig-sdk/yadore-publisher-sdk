...
describe('MarketEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set YADORE_PUBLISHER_TEST_MARKET_ENTID JSON to run live')
      return
    }
    ...
    const market_ref01_match: any = {}

    const market_ref01_list = (await market_ref01_ent.list(market_ref01_match)).map((e: any) => e.data())
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['YADORE_PUBLISHER_TEST_MARKET_ENTID']
  ...
  idmap = env['YADORE_PUBLISHER_TEST_MARKET_ENTID']
