...
local ReportGeneralEntity = {}
ReportGeneralEntity.__index = ReportGeneralEntity


function ReportGeneralEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ReportGeneralEntity)
  self._name = "report_general"
  self._client = client
...
function ReportGeneralEntity:get_name()
  return self._name
end


function ReportGeneralEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ReportGeneralEntity.new(self._client, opts)
end
...
function ReportGeneralEntity:mark_deleted()
  self._deleted = true
end


function ReportGeneralEntity:deleted()
  return true == self._deleted
end


function ReportGeneralEntity:data_set(args)
  if args ~= nil then
...
function ReportGeneralEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ReportGeneralEntity:match_set(args)
  if args ~= nil then
...
function ReportGeneralEntity:match_get()
...
function ReportGeneralEntity:stream(action, args, callopts)
...
---@param reqmatch ReportGeneralLoadMatch
---@param ctrl? table
---@return ReportGeneral
---@return string? err
function ReportGeneralEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ReportGeneralEntity:_run_op(ctx, post_done)
  local utility = self._utility
