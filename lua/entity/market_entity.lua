...
local MarketEntity = {}
MarketEntity.__index = MarketEntity


function MarketEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, MarketEntity)
  self._name = "market"
  self._client = client
...
function MarketEntity:get_name()
  return self._name
end


function MarketEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return MarketEntity.new(self._client, opts)
end
...
function MarketEntity:mark_deleted()
  self._deleted = true
end


function MarketEntity:deleted()
  return true == self._deleted
end


function MarketEntity:data_set(args)
  if args ~= nil then
...
function MarketEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function MarketEntity:match_set(args)
  if args ~= nil then
...
function MarketEntity:match_get()
...
function MarketEntity:stream(action, args, callopts)
...
function MarketEntity:_run_op(ctx, post_done)
  local utility = self._utility
