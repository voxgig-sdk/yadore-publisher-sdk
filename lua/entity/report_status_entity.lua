...
local ReportStatusEntity = {}
ReportStatusEntity.__index = ReportStatusEntity


function ReportStatusEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ReportStatusEntity)
  self._name = "report_status"
  self._client = client
...
function ReportStatusEntity:get_name()
  return self._name
end


function ReportStatusEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ReportStatusEntity.new(self._client, opts)
end
...
function ReportStatusEntity:mark_deleted()
  self._deleted = true
end


function ReportStatusEntity:deleted()
  return true == self._deleted
end


function ReportStatusEntity:data_set(args)
  if args ~= nil then
...
function ReportStatusEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ReportStatusEntity:match_set(args)
  if args ~= nil then
...
function ReportStatusEntity:match_get()
...
function ReportStatusEntity:stream(action, args, callopts)
...
---@param reqmatch ReportStatusLoadMatch
---@param ctrl? table
---@return ReportStatus
---@return string? err
function ReportStatusEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ReportStatusEntity:_run_op(ctx, post_done)
  local utility = self._utility
