...
local MerchantEntity = {}
MerchantEntity.__index = MerchantEntity


function MerchantEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, MerchantEntity)
  self._name = "merchant"
  self._client = client
...
function MerchantEntity:get_name()
  return self._name
end


function MerchantEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return MerchantEntity.new(self._client, opts)
end
...
function MerchantEntity:mark_deleted()
  self._deleted = true
end


function MerchantEntity:deleted()
  return true == self._deleted
end


function MerchantEntity:data_set(args)
  if args ~= nil then
...
function MerchantEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function MerchantEntity:match_set(args)
  if args ~= nil then
...
function MerchantEntity:match_get()
...
function MerchantEntity:stream(action, args, callopts)
...
function MerchantEntity:_run_op(ctx, post_done)
  local utility = self._utility
