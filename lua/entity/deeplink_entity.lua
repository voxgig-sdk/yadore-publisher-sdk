...
local DeeplinkEntity = {}
DeeplinkEntity.__index = DeeplinkEntity


function DeeplinkEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DeeplinkEntity)
  self._name = "deeplink"
  self._client = client
...
function DeeplinkEntity:get_name()
  return self._name
end


function DeeplinkEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DeeplinkEntity.new(self._client, opts)
end
...
function DeeplinkEntity:mark_deleted()
  self._deleted = true
end


function DeeplinkEntity:deleted()
  return true == self._deleted
end


function DeeplinkEntity:data_set(args)
  if args ~= nil then
...
function DeeplinkEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DeeplinkEntity:match_set(args)
  if args ~= nil then
...
function DeeplinkEntity:match_get()
...
function DeeplinkEntity:stream(action, args, callopts)
...
function DeeplinkEntity:_run_op(ctx, post_done)
  local utility = self._utility
