...
local DntEntity = {}
DntEntity.__index = DntEntity


function DntEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DntEntity)
  self._name = "dnt"
  self._client = client
...
function DntEntity:get_name()
  return self._name
end


function DntEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DntEntity.new(self._client, opts)
end
...
function DntEntity:mark_deleted()
  self._deleted = true
end


function DntEntity:deleted()
  return true == self._deleted
end


function DntEntity:data_set(args)
  if args ~= nil then
...
function DntEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DntEntity:match_set(args)
  if args ~= nil then
...
function DntEntity:stream(action, args, callopts)
...
---@param reqmatch DntLoadMatch
---@param ctrl? table
---@return Dnt
---@return string? err
function DntEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function DntEntity:_run_op(ctx, post_done)
  local utility = self._utility
