...
local OfferEntity = {}
OfferEntity.__index = OfferEntity


function OfferEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, OfferEntity)
  self._name = "offer"
  self._client = client
...
function OfferEntity:get_name()
  return self._name
end


function OfferEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return OfferEntity.new(self._client, opts)
end
...
function OfferEntity:mark_deleted()
  self._deleted = true
end


function OfferEntity:deleted()
  return true == self._deleted
end


function OfferEntity:data_set(args)
  if args ~= nil then
...
function OfferEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function OfferEntity:match_set(args)
  if args ~= nil then
...
function OfferEntity:match_get()
...
function OfferEntity:stream(action, args, callopts)
...
function OfferEntity:_run_op(ctx, post_done)
  local utility = self._utility
