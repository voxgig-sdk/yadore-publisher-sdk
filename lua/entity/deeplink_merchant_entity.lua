-- YadorePublisher SDK DeeplinkMerchant entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local DeeplinkMerchantEntity = {}
DeeplinkMerchantEntity.__index = DeeplinkMerchantEntity


function DeeplinkMerchantEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DeeplinkMerchantEntity)
  self._name = "deeplink_merchant"
  self._client = client
...
function DeeplinkMerchantEntity:get_name()
  return self._name
end


function DeeplinkMerchantEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DeeplinkMerchantEntity.new(self._client, opts)
end
...
function DeeplinkMerchantEntity:mark_deleted()
  self._deleted = true
end


function DeeplinkMerchantEntity:deleted()
  return true == self._deleted
end


function DeeplinkMerchantEntity:data_set(args)
  if args ~= nil then
...
function DeeplinkMerchantEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DeeplinkMerchantEntity:match_set(args)
  if args ~= nil then
...
function DeeplinkMerchantEntity:match_get()
...
function DeeplinkMerchantEntity:stream(action, args, callopts)
...
function DeeplinkMerchantEntity:_run_op(ctx, post_done)
  local utility = self._utility
